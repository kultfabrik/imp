...
const TitleScreen = {
  ...
  ]); if (!Chat.msgs.length) { Chat.sys('Willkommen im Chat von imp'); Chat.say('hafi!', ['hafermilchHans', '#ff7f50']); Chat.say('wann geht der stream los', ['sub_sabine', '#ff5ad1']); Chat.say('er ist zu spät. wie immer.', ['ludwig_fan_01', '#7bd23a']); } },
  ...
  draw(g) {
    ...
    Sprites.draw(g, Math.floor(this.t / 12) % 2 ? 'ludwig_walk' : 'ludwig_idle', wx - 22, VH - 32 - 28, Math.cos(this.t * 0.02) < 0);
...
const ScenePlayer = {
  ...
  draw(g) {
    ...
    if (this.bg === 'room' || !Game.level) { Backgrounds.draw(g, 'hub', 40, 0, VW, VH, false); Tiles.set('hub'); for (let x = 0; x < VW; x += 16) { g.drawImage(Tiles.img('#', 'top'), x, VH - 16); } Sprites.draw(g, 'kasten', 250, VH - 48); Sprites.draw(g, 'marx_bust', 200, VH - 32); Sprites.draw(g, 'box', 20, VH - 48); Sprites.draw(g, 'box', 46, VH - 48); Sprites.draw(g, 'imp_idle', 120, VH - 40); Sprites.draw(g, 'ludwig_idle', 150, VH - 44, true); }
    else dim(g, 0.35);
...
const IntroCard = {
  ...
  draw(g) { const L = this.L; g.fillStyle = '#0e0e10'; g.fillRect(0, 0, VW, VH); Backgrounds.draw(g, L.theme, this.t * 0.4, 0, VW, VH, false); dim(g, 0.55);
    ...
    Sprites.draw(g, 'face_ludwig', 30, 144); Font.draw(g, 'Ludwigs Tipp:', 60, 144, '#7bd23a'); Font.drawWrapped(g, L.hints[0], 60, 154, 40, '#efeff1');
...
const GameOverScreen = {
  ...
  draw(g) { dim(g, 0.85); Font.drawBig(g, 'STREAM', VW / 2, 50, '#e03a3a', 3, { align: 'center', shadow: true }); Font.drawBig(g, 'ABGEBROCHEN', VW / 2, 78, '#e03a3a', 2, { align: 'center', shadow: true });
    Font.draw(g, 'Keine BRBs mehr. Der Chat schreibt "F".', VW / 2, 110, '#efeff1', { align: 'center' }); Font.draw(g, 'Ludwig: "Wir versuchen es morgen. Also heute. Später."', VW / 2, 122, '#7bd23a', { align: 'center' });
...
const SpaetiShop = {
  ...
  update() { this.menu.update(); },
  draw(g) { dim(g, 0.6); box(g, 30, 30, VW - 60, 150, '#1a1a22', '#7bd23a'); Font.draw(g, 'SPÄTI24 - seit 3 Uhr nachts', VW / 2, 36, '#7bd23a', { align: 'center' }); Font.draw(g, 'Bits: ' + Game.bits + (Game.level.player.hasPerk('prime') && !Game.primeUsed ? ' · Prime: 1 Gratis-Item' : ''), VW / 2, 46, '#c9a4ff', { align: 'center' }); this.menu.draw(g, 50, 60, VW - 110, 11); Sprites.draw(g, 'ludwig_idle', VW - 60, 100); }
};
/* ====================== HUB STATIONS ====================== */
const STATION_INFO = { map: ['Twitch-Monitor', 'Weltkarte & Level starten'], vods: ['YouTube-Monitor', 'VODs, Statistik, Trophäen'], ludwig: ['Ludwig', 'Perk-Shop & Verträge'], sinan: ['Sinan', 'Trainingsraum'], marx: ['Marx-Büste', 'Hinweis zum nächsten Level'], fridge: ['Kühlschrank', 'Start-Item einpacken'] };
class HubStation extends Entity {
  ...
  activate() {
    switch (this.kind) {
      case 'map': UI.open(WorldMap); break; case 'vods': UI.open(VodScreen); break; case 'ludwig': UI.open(PerkShop); break; case 'fridge': UI.open(FridgeMenu); break;
  ...
  draw(g) {
    ...
    switch (this.kind) {
      ...
      case 'ludwig': Sprites.draw(g, this.anim % 60 < 30 ? 'ludwig_idle' : 'ludwig_walk', this.x, this.y - 4, p && p.cx < this.cx); if (Save.slot && Save.slot.bits >= 150 && this.anim % 50 < 25) Font.draw(g, '!', this.cx, this.y - 14, '#ffd700', { align: 'center', shadow: true }); break;
...
HUB_DEF.ents = [
  { t: 'station', x: 5, y: 12, kind: 'map' }, { t: 'station', x: 11, y: 12, kind: 'vods' }, { t: 'station', x: 17, y: 12, kind: 'ludwig' }, { t: 'station', x: 24, y: 12, kind: 'sinan' }, { t: 'station', x: 30, y: 12, kind: 'marx' }, { t: 'station', x: 36, y: 12, kind: 'fridge' },
...
const PerkShop = {
  ...
  act(it) { const s = Save.slot; const id = it.id; if (!s.perks.includes(id)) { const cost = PERKS[id].cost; if (s.bits < cost) { AudioSys.sfx('back'); Game.banner('Zu wenig Bits. Ludwig: "Ich kann dir nichts vorstrecken."', '#e03a3a'); return; } s.bits -= cost; s.perks.push(id); AudioSys.sfx('power'); Game.banner('Perk gekauft: ' + PERKS[id].name, '#7bd23a'); }
  ...
  draw(g) { const s = Save.slot; dim(g, 0.75); twitchHeader(g, 'LUDWIG · ' + (this.tab === 0 ? 'PERK-SHOP' : 'VERTRAGSORDNER') + '   (←/→ wechseln)', 4);
    ...
    else { box(g, 4, 16, VW - 8, 220); Font.draw(g, 'Unterschriebene Werbepartner: ' + s.sponsors.length + '/10', 10, 22, '#ffd700'); for (let i = 1; i <= 10; i++) { const sp = SPONSORS[i]; const has = s.sponsors.includes(sp.name); Font.draw(g, (has ? '✓ ' : '· ') + sp.name, 12, 34 + (i - 1) * 16, has ? '#7bd23a' : '#5a5a66'); Font.draw(g, has ? '"' + sp.slogan + '"' : '???', 22, 42 + (i - 1) * 16, has ? '#c8c8d0' : '#3a3a44'); } Sprites.draw(g, 'ludwig_idle', VW - 40, 200); Font.draw(g, 'Ludwig: "Alles rechtssicher. Glaub ich."', VW - 14, 224, '#7bd23a', { align: 'right' }); } }
};
/* ====================== FRIDGE ====================== */
const FridgeMenu = {
  menu: null,
