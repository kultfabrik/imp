...
class NPC extends Entity { constructor(x, y, who, lines) { super(x, y - 24, 14, 24); this.who = who; this.lines = lines; this.type = 'npc'; this.idx = 0; this.cd = 0; } update() { this.anim++; if (this.cd > 0) this.cd--; const p = this.player; if (p && rectHit({ x: this.x - 12, y: this.y, w: 38, h: 24 }, p) && Input.was('up') && this.cd <= 0) { this.cd = 30; Game.speech(this, this.lines[this.idx % this.lines.length], 150); this.idx++; } } draw(g) { const spr = { luigi: 'luigi_idle', sinan: 'sinan_idle', marx: 'marx_idle', baba: 'baba_small', imp: 'imp_idle' }[this.who] || 'luigi_idle'; Sprites.draw(g, spr, this.x - 1, this.who === 'baba' ? this.y + 8 : this.y, this.player && this.player.cx < this.cx); if (this.player && Math.abs(this.player.cx - this.cx) < 30 && this.anim % 40 < 20) Font.draw(g, '↑ reden', this.cx, this.y - 10, '#fff', { align: 'center', shadow: true }); } }
